 ...
 * 		Delete、End、PageDown、NumLock和方向键（Left、Up、Right、Down）外的其他键;
 * 		键值范围: 13回车 32 - 126
 * 连续键、控制组合键、单键在keydown事件的互斥触发：优先级(连续键>控制组合键>单键)
 * 当当前键盘操作结果为 (可触发连续键或可做为触发连续键的必要条件) 时屏蔽掉 (能够触发的控制组合键和单键);
 * 其他类推.
 * 
 * 注意：当对单个非可打印字符键进行事件绑定时，建议绑定keydown/keyup类型的事件（keypress存在问题）
 * 
...
STK.register('core.evt.hotKey', function($) {
	...
	var F = {
		...
		//特殊键的对照
		,keyMap: { 27: 'esc', 9: 'tab', 32:'space', 10: 'enter', 13: 'enter', 8:'backspace', 145: 'scrollclock', 
	...
	var that = {
		 ...
		 * 	    };
		 * 	    STK.core.evt.hotKey.add(document.documentElement, ["0","ctrl+9"], f1);
		 */
		 ...
		 *		};
		 * 		STK.core.evt.hotKey.remove(document.documentElement, ["0","ctrl+9"], f1);
		 * @return {void}
		 */
