...
;( function(root, factory) {
		...
		SVG.prepare = function(element) {
			...
			var body = document.getElementsByTagName('body')[0], draw = ( body ? new SVG.Doc(body) : element.nested()).size(2, 0), path = SVG.create('path')
		...
		SVG.extend(SVG.Number, {
			// Stringalize
			toString : function() {
				return (this.unit == '%' ? ~~(this.value * 1e8) / 1e6 : this.unit == 's' ? this.value / 1e3 : this.value
				) + this.unit
		...
		SVG.ViewBox = function(element) {
			var x, y, width, height, wm = 1/* width multiplier */, hm = 1/* height multiplier */, box = element.bbox(), view = (element.attr('viewBox') || '').match(/-?[\d\.]+/g), we = element, he = element
			...
			extend : {
				...
				attr : function(a, v, n) {
					if (a == null) {
						...
						return v == null ? SVG.defaults.attrs[a] : SVG.regex.isNumber.test(v.nodeValue) ? parseFloat(v.nodeValue) : v.nodeValue
		...
		SVG.FX = SVG.invent({
			...
			extend : {
				...
				animate : function(d, ease, delay) {
					var akeys, tkeys, skeys, key, element = this.target, fx = this
						...
						this.timeout = setTimeout(function() {
							...
							fx.render = function() {
								...
								if (fx.situation.play === true) {
									...
									var time = new Date().getTime(), pos = time > fx.situation.finish ? 1 : (time - fx.situation.start) / d
				...
				attr : function(a, v) {
					if ( typeof a == 'object') {
						...
						this.attrs[a] = SVG.Color.isColor(from) ? new SVG.Color(from).morph(v) : SVG.regex.unit.test(from) ? new SVG.Number(from).morph(v) : {
							from : from,
							to : v
						}
					}
		...
		SVG.extend(SVG.Text, SVG.TSpan, {
			...
			,
			tspan : function(text) {
				var node = (this.textPath || this).node, tspan = new SVG.TSpan
